...
export const Navbar = () => {
  ...
  // أضف هذا الـ useEffect بعد الـ hooks
  useEffect(() => {
    // التعامل مع الـ hash عند تحميل الصفحة
    if (window.location.hash === "#offers" && !isHome) {
      // إذا كان الرابط يحتوي على #offers ونحن لسنا في الهوم
      // نوجه إلى الهوم مع الـ hash
      window.location.href = `/${language}#offers`;
    }

    // التعامل مع السكرول عند وجود hash في الهوم
    if (isHome && window.location.hash === "#offers") {
      setTimeout(() => {
